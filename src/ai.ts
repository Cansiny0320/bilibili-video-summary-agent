...
export class AIService {
  ...
  private async transcribeWithVolc(filePath: string, timeOffset: number): Promise<SubtitleItem[]> {
    try {
      ...
      if (errorMsg && errorMsg.includes('no available instances')) {
        console.warn(
          `Volc cluster '${
            process.env.VOLC_CLUSTER || 'volc_auc_common'
          }' not found/unavailable, retrying without cluster...`,
        )
      ...
      if (errorMsg && errorMsg.includes('is not allowed')) {
        ...
        console.error(
          'Please ensure your App ID has "Short Audio Recognition" (一句话识别) or "Flash Recognition" (极速版) enabled.',
        )
        console.error(
          'Note: "volc.bigasr.auc" is typically for Async API (requires URL) and cannot be used with local files here.\n',
        )
      }
  ...
  private async transcribeWithVolcFallback(
    filePath: string,
    timeOffset: number,
